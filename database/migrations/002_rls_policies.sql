-- Omar Clinic Pro - Row Level Security (RLS) Policies V2 (CORRECTED)
-- Migration: 002_rls_policies_v2_corrected.sql
...
-- CORRECTED: Fixed role escalation policy
-- Instead of USING (false) which blocks all updates, use proper role check
-- Allow updates only if the role_id is not being changed
...
CREATE POLICY users_no_role_escalation ON users
  ...
  WITH CHECK (
    clinic_id = get_user_clinic_id() AND
    (is_clinic_owner() OR is_super_admin()) AND
    role_id = (SELECT role_id FROM users WHERE id = users.id)
  );
...
-- ============================================================================
-- Migration: 002_rls_policies_v2_corrected.sql
-- Version: CORRECTED - Fixed role escalation policy
--
-- Issues Fixed:
--   1. ✅ Fixed users_no_role_escalation policy (proper role check instead of USING false)
--
-- RLS Policies Implemented:
--   - Multi-tenant clinic isolation on all tables
--   - Role-based access control (5 roles)
--   - Soft delete filtering (deleted_at IS NULL)
--   - Privilege escalation prevention (proper implementation)
